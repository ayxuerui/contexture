...
The pre-push hook refuses a direct push to `__DEFAULT_BRANCH__`; `CONTEXTURE_ALLOW_DEFAULT_BRANCH_PUSH=1` is its emergency override — for genuine emergencies, not routine use. A generated harness permission config (for example, a harness-generation adapter's `.claude/settings.json`) is committed, so it names the primitive it invokes — such as the write-gate hook's command — through the harness's own project-root placeholder rather than through a filesystem path resolved when the file was generated. The generated file is therefore identical on every machine, and the command resolves against whichever checkout the session actually opened, canonical or worktree. Only a harness that publishes no such placeholder falls back to an absolute path, anchored at the store's main worktree and never at whichever checkout happened to run the generator.
