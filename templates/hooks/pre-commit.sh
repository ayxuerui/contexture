...
if [ ! -f "$CONTEXTURE_BIN" ]; then
  echo "contexture: warning — could not find $CONTEXTURE_BIN; skipping pre-commit validation." >&2
  exit 0
fi

if ! output=$(node "$CONTEXTURE_BIN" doctor --staged --json 2>&1); then
  echo "$output" >&2
  echo "contexture: commit refused — staged changes failed 'doctor --staged'." >&2
  exit 1
fi
