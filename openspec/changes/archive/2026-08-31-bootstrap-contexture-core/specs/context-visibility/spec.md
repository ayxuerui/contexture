...
Governs which named context may retrieve a given note. The frontmatter key carrying this label is the visibility field defined by `context-store` (provisionally `scope:`); this capability never names that key directly and refers throughout to "the visibility field."
