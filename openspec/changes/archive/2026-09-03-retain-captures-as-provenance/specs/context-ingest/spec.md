...
**Reason**: The rule was absolute in both directions and neither half survives. It forbade a capture from arriving with any identity field, which no real capture pipeline can honour — a fetcher knows the source type and source id at the moment it writes the file. And it placed the assigned identity on "the resulting note", which is the decision this change reverses: identity now belongs to the retained capture, so a note synthesized from several captures can cite each one. Its second scenario also guarded against the inbox file being mistaken for a duplicate of the note it produced, a hazard that disappears once the note carries no identity at all.

**Migration**: Replaced by "Ingest assigns the content hash and the ingested date", which narrows the prohibition to the two fields ingest actually owns, and by "Ingest retains the capture and cites it from a note", which states where identity lands. Notes carrying identity fields from before this change keep them and remain valid dedupe records under "Two-stage content-addressed dedupe"; nothing is rewritten retroactively.
