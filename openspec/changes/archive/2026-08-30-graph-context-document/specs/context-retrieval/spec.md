...
Extends the context-retrieval capability (see `bootstrap-contexture-core`): the graph leg renders a human-readable document, clusters its nodes positionally, records typed edges from relation sections named by a configured vocabulary, and answers cluster and bridge queries. The graph still enumerates structure and ranks nothing.
