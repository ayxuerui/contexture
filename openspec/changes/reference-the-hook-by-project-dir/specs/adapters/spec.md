...
When the enforcement primitive is invoked by a path (for example, a hook command), and the target harness publishes a placeholder naming the root of the project it has open, `contexture adapters generate` SHALL express that path using the harness's placeholder rather than resolving it at generation time — so the generated config carries no machine-specific absolute path and is byte-identical regardless of which machine or checkout generated it. The generator SHALL NOT express the path relative to the primitive's working directory, which the harness does not guarantee to be the project root. Where the harness publishes no such placeholder, `contexture adapters generate` SHALL resolve an absolute path against the store's main/canonical worktree, never against whichever checkout is currently running the generator, so the primitive keeps resolving after a session worktree that once ran the generator is removed. A primitive that fails to resolve SHALL NOT be treated as equivalent to a passing enforcement decision — an enforcement primitive that cannot run MUST NOT cause the edit it would have evaluated to be silently allowed.
...
#### Scenario: The generated invocation carries no machine-specific path
- **WHEN** `contexture adapters generate` runs on two different machines whose store checkouts live at different filesystem locations, for a harness that publishes a project-root placeholder, against otherwise identical store configuration
- **THEN** the path the generated permission config invokes the enforcement primitive by is byte-identical on both machines, and contains neither store's filesystem location
...
- **THEN** the path the generated config invokes still resolves — for a harness publishing a placeholder, because the path names no checkout at all and is expanded against whichever project the harness has open; otherwise because it was resolved against the store's main worktree rather than the worktree that ran the generator

#### Scenario: A placeholder-expressed primitive resolves in whichever checkout the session opened
- **WHEN** a session opens a session worktree as its project root, and the generated config invokes the primitive through the harness's project-root placeholder
- **THEN** the primitive resolves to that worktree's own copy of the script, and a session opened on the canonical checkout instead resolves to the canonical checkout's copy
...
For a rule whose enforcement primitive is invoked by a path, `contexture adapters generate` SHALL additionally remove any existing rule that matches the current rule in every respect except the path the primitive is invoked by, before writing the current rule — so a permission config that has accumulated one such rule per checkout that has ever run the generator, or that still carries a path a previous version resolved at generation time, converges to exactly one rule on the next run, not only when the generator's own version changes.
...
- **WHEN** a permission config already contains a rule this same generator emitted on an earlier run, whose only difference from the rule the current run would emit is the path baked into it
...
#### Scenario: A generation-time absolute path is replaced by the placeholder form
- **WHEN** a permission config carries an enforcement-primitive invocation that a previous version of the generator resolved to an absolute path, and the current version emits the same primitive through the harness's project-root placeholder
- **THEN** running `contexture adapters generate` leaves exactly one such rule in the file, expressed through the placeholder, with the previously resolved absolute path absent
