...
const CONFIG_DELTA: MigrationDelta = {
  ...
  description: 'remove the visibility:, disclosure:, and fields: blocks, and set schema_version to 7',
};
 ...
 *
 * Pending-ness is `schema_version < 7`, not key presence — the rule every
...
export const dropAccessAxesMigration: Migration = {
  id: '0007-drop-access-axes',
  fromVersion: 6,
  toVersion: 7,
  description:
    'Remove the visibility:, disclosure:, and fields: blocks from contexture.yaml, and bump schema_version to 7. No note is modified.',

  async plan(store) {
    if (store.config.schema_version >= 7) return [];
    return [CONFIG_DELTA];
  },
  ...
  async apply(store) {
    ...
    const nextConfig: Record<string, unknown> = { ...currentConfig, schema_version: 7 };
