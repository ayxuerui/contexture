...
const SESSION_LIFECYCLE: ProcedureSeed = {
  ...
  description: 'Start a session worktree, re-scan before planning, stage surgically, submit and land behind explicit gates, verify before any retry, and resolve conflicts.',
  body: (config) => {
    ...
    return [
      ...
      `branch (\`${defaultBranch}\`). This skill owns start → submit → land end to end — no other skill instructs a`,
      'direct commit, push, or merge.',
      '',
      '## Start',
      '',
      ...
      '   under you while you work; a plan that grows between scans is normal. Name the delta from the previous',
      '   scan rather than silently folding new work into old buckets.',
      '2. Capture pass: run `ctxr-session-capture` exactly once here (do not fire it again at submit).',
      ...
      '## Fire gate — submit',
      '',
      '`ctxr session submit` commits, pushes, and opens the pull request: that is an external side effect, and',
      'plan consent is not fire consent. Present the branch, the title, and what rides it; wait for an explicit',
      'go; then run `ctxr session submit --title "<title>" --body "<why / what changed / verification /',
      'follow-ups>"`. It runs the full store validation first — fix a failure, never bypass it. If no forge is',
      'reachable it pushes and prints the manual pull-request instructions.',
      '',
      '## Verify before any retry',
      '',
      'A transport error can arrive AFTER the push or the pull-request open already succeeded. Before retrying',
      'anything: `git ls-remote origin <branch>` and the forge\'s pull-request list for the branch. Never replay',
      'a push or a pull-request open blindly.',
      '',
      '## Fire gate — land',
      '',
      'Merging mutates the default branch. Until `ctxr session land` ships, the manual equivalent: fetch, then',
      'read the pull request\'s state on the forge. OPEN and mergeable → present its number and title, wait for',
      'an explicit go, merge (squash unless asked otherwise), and confirm the state reads merged afterwards.',
      'Already merged → skip to sync. Closed → stop and report. Conflicting → the playbook below. Then sync the',
      `root checkout with a fast-forward only: \`git fetch origin && git merge --ff-only origin/${defaultBranch}\`;`,
      'a checkout that will not fast-forward is reported, never forced.',
      '',
      '## Fire gate — reclaim',
      '',
      '`ctxr session reap` removes merged, clean worktrees; `ctxr session abandon <branch>` discards work and',
      'needs an explicit go. Never claim cleanup happened without having run one of them.',
      '',
      '## Conflict playbook',
      '',
      ...
      'overlap: wait for the merge, then start the next session off the updated default branch. Worktrees',
      'isolate directories, not logical writes: a hot file every session appends to collides at merge time',
      'regardless — sequence those.',
    ];
  },
};
