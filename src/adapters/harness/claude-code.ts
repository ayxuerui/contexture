...
import type { GeneratedSkillFile, HarnessGenerationAdapter } from '../types.js';
...
export const claudeCodeHarnessAdapter: HarnessGenerationAdapter = {
  ...
  /**
   * contexture-home-layout spec: one thin wrapper per canonical procedure so
   * Claude Code's skill auto-discovery surfaces it. Frontmatter + a pointer —
   * the canonical procedure file stays the single source; editing it never
   * requires regenerating the wrapper.
   */
  renderSkills(procedures): GeneratedSkillFile[] {
    return procedures.map((procedure) => {
      const slug = path.basename(procedure.path, '.md');
      return {
        path: `.claude/skills/contexture-${slug}/SKILL.md`,
        content: [
          '---',
          `name: contexture-${slug}`,
          `description: ${procedure.description}`,
          '---',
          '',
          `<!-- Generated by \`contexture adapters generate\` — do not edit. The canonical procedure is the file below. -->`,
          '',
          `Read and follow \`${procedure.path}\` (relative to the store root). That file is the`,
          `single source of truth for the "${procedure.name}" procedure; this skill exists only so`,
