...
describe('session land (session-submit-and-land D1): the state machine', () => {
  ...
  it('a root checkout off the default branch is reported, not forced', async () => {
    ...
    expect(outcome.data?.sync).toEqual({
      attempted: false,
      synced: false,
      reason: 'root checkout is not on the default branch',
    });
  });
...
describe('session land --reap (D3): opt-in and scoped', () => {
  ...
  it('refuses to remove the worktree it is currently running from, even after its branch was renamed', async () => {
    // Realistic self-referential case: `session land` invoked from INSIDE the worktree it would otherwise reap,
    // so store.root IS that worktree's path, and its branch has been renamed away from the session/ prefix.
    const selfStore: Store = { root: '/repo/.worktrees/session-abc', config: makeConfig() };
    const { adapter } = fakeForge({ initial: OPEN_MERGEABLE });
    const porcelain = ['worktree /repo/.worktrees/session-abc', 'HEAD aaaa', 'branch refs/heads/topic/x', ''].join('\n');
    const { git, calls } = fakeGitRunner(
      new Map([
        ['symbolic-ref --short HEAD', { exitCode: 0, stdout: 'topic/x\n', stderr: '' }], // this checkout is on the session branch, not main
        ['worktree list --porcelain', { exitCode: 0, stdout: porcelain, stderr: '' }],
      ]),
    );
    const env = nonInteractiveEnv({ git });
    ...
    expect(outcome.data?.sync).toEqual({ attempted: false, synced: false, reason: 'root checkout is not on the default branch' });
    expect(outcome.data?.reap.reaped).toBe(false);
    expect(outcome.data?.reap.reason).toContain('currently running from');
